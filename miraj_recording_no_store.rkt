...
(define (append-def (context Context?) (b Binding?) (def-sto Store?)) Context?
           ...
           [bind (name loc)
                 (let ([where (new-loc sto)])
                   (e*s (cons (bind name where) env) 
                        (override-store sto where (fetch def-sto loc))))])]))
  ...
  (cond
    ...
    [else 
     (type-case JoinPoint (list-box-pop! jps)
       ...
       [app-return (name abs result jp-sto) 
                   (v*s result sto)])]))
